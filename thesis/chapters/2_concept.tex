...
\begin{itemize}
  ...
  \item milyen, más operációs rendszerekben megjelenő megoldások (pl.\ OpenBSD
        pledge és unveil) szolgálnak koncepcionális
        előképként,
...
A fejezet áttekinti a kapcsolódó
...
Erre a problémakörre válaszként jelentek meg különböző sandboxolási
...
A Unix világban régóta léteznek olyan primitívek, mint a chroot
\cite{linux_chroot_2} vagy
...
\cite{docker_engine_security} \cite{lxc_introduction}
...
\subsection{OpenBSD pledge és unveil}

Az OpenBSD egyik fontos újítása a pledge és unveil
mechanizmus bevezetése volt.
A pledge lényege, hogy a folyamat
...
nem pledge-elt, a kernel azonnal leállítja. \cite{openbsd_pledge_2}

Az unveil ezzel párhuzamosan a fájlrendszer eléréséhez kínál egy
...
szűkített nézetet kap. \cite{openbsd_unveil_2}

Ezek a megoldások fontos koncepcionális példát jelentenek: a biztonsági
mechanizmusok fejlesztő-barát, jól érthető API-n keresztül válnak
...
követi: a seccomp rész nagy vonalakban a pledge, a
Landlock rész pedig az unveil szerepét tölti be.
...
legújabb generációs Landlock LSM. \cite{linux_landlock_lsm}
...
mint a fájlmegnyitás, hálózati kapcsolat létrehozása vagy folyamatok
létrehozása \cite{linux_lsm_hooks}. A SELinux például összetett címkézési
és szabályrendszert használ, és többek között Type Enforcement, RBAC és MLS
alapú kötelező hozzáférés-vezérlési modelleket támogat
\cite{selinux_8}. Az AppArmor ezzel szemben
profilokra épülő, útvonal-alapú hozzáférés-vezérlési modellt alkalmaz
\cite{linux_apparmor_lsm}.
...
A seccomp mechanizmus lehetővé teszi, hogy egy folyamat a futása
során ``lezárja'' magát, és csak egy szűkített rendszerhívás-halmazt használhasson.
A modern seccomp-bpf változatban a kernel egy BPF programot futtat
...
A dolgozatban bemutatott könyvtár seccomp modulja éppen ezt az
...
használható LSM-modul, amely elsősorban a fájlrendszer-hozzáférés
korlátozására koncentrál. A folyamat egy úgynevezett ruleset-et hoz
...
korlátozások beállításához: a folyamat a saját jogain belül szabadon
...
\code{landlock\_\allowbreak ruleset\_\allowbreak attr}, \code{landlock\_\allowbreak path\_\allowbreak beneath\_\allowbreak attr}
...
absztrakciók mögé, mint a \code{LandlockAccess} enum class vagy a
\code{LandlockRuleSet} RAII-típus.
...
\begin{itemize}
  \item libseccomp: C nyelvű könyvtár, amely megkönnyíti a
  ...
  \item Firejail, Bubblewrap, Flatpak stb.: felhasználói szintű
  ...
  \item Disztribúció-specifikus profilrendszerek: SELinux- vagy
...
olyan, modern C++-ra szabott, könnyen beépíthető könyvtár, amely egységes
...
\begin{description}
  \item[Fejlesztőbarát API]
  Hasonlóan az OpenBSD pledge / unveil pároshoz, a könyvtárnak
  néhány jól érthető, magas szintű primitívet kell kínálnia, amelyek mögött
  a bonyolult kernelmechanizmusok elrejthetők.

  \item[Moduláris felépítés]
  A rendszerhívás-szintű, seccomp alapú és a fájlrendszer-szintű, Landlock
  alapú korlátozást logikailag külön, de összehangolható módon kell kezelni.

  \item[Modern C++ gyakorlatok]
  A könyvtár RAII-alapú erőforráskezelést, \code{std::\allowbreak expected}-alapú
  hibakezelést, erős típusosságot, kivételmentes működést és átlátható
  erőforrás-életciklusokat használ.

  \item[Biztonságos alapértelmezések]
  A könyvtár ésszerű alapértelmezett konfigurációkat kínál, például
  alapértelmezett Landlock-útvonalakat, hogy minimális beállítással is
  értelmezhető védelmi szint legyen elérhető.

  \item[Kombinálhatóság]
  A seccomp és a Landlock mechanizmusok olyan módon használhatók együtt, hogy
  egymást kiegészítve, többrétegű védelmet biztosítsanak egy támadóval szemben.
\end{description}
...
\begin{itemize}
  \item Milyen, az OpenBSD pledge / unveil megoldásához
