...
\begin{itemize}
  ...
  \item milyen, más operációs rendszerekben megjelenő megoldások (pl.\ OpenBSD
        \code{pledge} és \code{unveil}) szolgálnak koncepcionális
        előképként,
  \item milyen konkrét technológiák érhetők el Linuxon (\code{seccomp}, SELinux,
...
A Unix világban régóta léteznek olyan primitívek, mint a \code{chroot}
...
Az OpenBSD egyik fontos újítása a \code{pledge} és \code{unveil}
mechanizmus bevezetése volt.
A \code{pledge} lényege, hogy a folyamat
...
Az \code{unveil} ezzel párhuzamosan a fájlrendszer eléréséhez kínál egy
...
követi: a \code{seccomp} rész nagy vonalakban a \code{pledge}, a
\code{Landlock} rész pedig az \code{unveil} szerepét tölti be.
...
A \code{seccomp} mechanizmus lehetővé teszi, hogy egy folyamat a futása
során ``lezárja'' magát, és csak egy szűkített rendszerhívás-halmazt használhasson.
A modern \code{seccomp-bpf} változatban a kernel egy BPF programot futtat
...
A dolgozatban bemutatott könyvtár \code{seccomp} modulja éppen ezt az
...
\begin{itemize}
  \item \code{libseccomp}: C nyelvű könyvtár, amely megkönnyíti a
  ...
  \item \code{Firejail}, \code{Bubblewrap}, \code{Flatpak} stb.: felhasználói szintű
...
\begin{itemize}
  \item \emph{Fejlesztő-barát API}: hasonlóan az OpenBSD \code{pledge} /
        \code{unveil} pároshoz, a könyvtárnak néhány jól érthető, magas
        szintű primitivet kell kínálnia, amelyek mögött a bonyolult kernel-
        mechanizmusok elrejthetők.
  \item \emph{Moduláris felépítés}: a rendszerhívás-szintű (\code{seccomp})
        és a fájlrendszer-szintű (Landlock) korlátozást logikailag
        külön, de összehangolható módon kell kezelni.
  \item \emph{Modern C++ gyakorlatok}: RAII, \code{std::expected}-alapú
        hibakezelés, erős típusosság, kivételmentes működés, átlátható
        erőforrás-életciklusok.
  \item \emph{Biztonságos alapértelmezések}: ésszerű default konfigurációk
  ...
  \item \emph{Kombinálhatóság}: a seccomp és Landlock mechanizmusok olyan
...
\begin{itemize}
  \item Milyen, az OpenBSD \code{pledge} / \code{unveil} megoldásához
