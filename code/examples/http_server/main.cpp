...
int main()
{
	...
	try {
		set_no_new_privs().value();

		// allow reading of "www" only
		auto ll = LandlockRuleSet::init().value();
		ll.add_rule("www", LandlockAccess::Read).value();
		ll.apply().value();

		// allow IO and network
		auto sc = SeccompBuilder::init().value();
		sc.allow("io").value();
		sc.allow("file_system").value();
		sc.allow("network_io").value();
		sc.build().value().view().apply().value();
	} catch (std::bad_expected_access<int> const& e) {
		std::cerr << "error: " << std::strerror(e.error()) << '\n';
		return 1;
	}
